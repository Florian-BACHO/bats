...
compute_deltas_kernel = cp.ElementwiseKernel("float32 grad, float32 m_hat, float32 v_hat, float32 learning_rate,"
                                             "float32 epsilon, float32 beta_1, "
                                             "float32 one_minus_beta_1, float32 one_minus_beta_1_power_t",
                                             "float32 delta",
                                             "delta = -(learning_rate / (sqrtf(v_hat) + epsilon)) *"
                                             "(beta_1 * m_hat + one_minus_beta_1 * grad / one_minus_beta_1_power_t)",
                                             "compute_deltas_kernel")


class NAdamOptimizer(AbstractOptimizer):
    ...
    def step(self, gradient: List[cp.ndarray]) -> List[cp.ndarray]:
        ...
        return [None if g is None else compute_deltas_kernel(g, m, v, self._learning_rate, self.__epsilon,
                                                             self.__beta_1, self.__one_minus_beta_1,
                                                             one_minus_beta_1_power_t)
